...
export default function Solutions() {
    ...
    return (
        ...
        <aside id="solution" className="flex flex-col lg:w-2/5 flex-none">
            <div className="flex-grow px-6 sm:px-10 py-8 bg-emerald-300 text-gray-800">
                <button onClick={()=>setShowSolutions(false)} className="absolute top-3 right-4 leading-none text-emerald-500 hover:text-emerald-700 text-sm">
                    <FontAwesomeIcon icon={faTimes}/>
                </button>

                {solutions.map((solution, index) => (
                    <div key={index}>
                        <Solution solution={solution} canExecute={canExecuteSolutions} isOpen={index === 0} isCollapsible={solutions.length >  1} />
        ...
        :
        <button onClick={()=>setShowSolutions(true)} className="
